...
export class Cluster {
	...
	constructor() {
		// @ts-ignore
		console.logMoreInfo = (logger: string, str: unknown) => { if (process.send) process.send({ op: "log", msg: `${typeof str == 'string' ? str : inspect(str)}`, logger: logger, source: `Cluster ${this.clusterID}` }) }
		console.log = (str: unknown) => {if (process.send) process.send({op: "log", msg: `${typeof str == 'string' ? str : inspect(str)}`, source: "Cluster " + this.clusterID});};
		console.debug = (str: unknown) => {if (process.send) process.send({op: "debug", msg: `${typeof str == 'string' ? str : inspect(str)}`, source: "Cluster " + this.clusterID});};
		console.error = (str: unknown) => {if (process.send) process.send({op: "error", msg: `${typeof str == 'string' ? str : inspect(str)}`, source: "Cluster " + this.clusterID});};
		console.warn = (str: unknown) => {if (process.send) process.send({op: "warn", msg: `${typeof str == 'string' ? str : inspect(str)}`, source: "Cluster " + this.clusterID});};
	...
	private async connect() {
		// @ts-ignore
		if (this.whatToLog.includes("cluster_start")) console.logMoreInfo('cluster_start', `Connecting with ${this.shards} shard(s)`)
		...
		bot.on("connect", (id: number) => {
			// @ts-ignore
			if (this.whatToLog.includes("shard_connect")) console.logMoreInfo('shard_connect', `Shard ${id} connected!`);
		});

		bot.on("shardDisconnect", (err: Error, id: number) => {
			// @ts-ignore
			if (!this.shutdown) if (this.whatToLog.includes("shard_disconnect")) console.logMoreInfo('shard_disconnect', `Shard ${id} disconnected with error: ${inspect(err)}`);
		});
		...
		bot.on("shardReady", (id: number) => {
			// @ts-ignore
			if (this.whatToLog.includes("shard_ready")) console.logMoreInfo('shard_ready', `Shard ${id} is ready!`);
		});

		bot.on("shardResume", (id: number) => {
			// @ts-ignore
			if (this.whatToLog.includes("shard_resume")) console.logMoreInfo('shard_resume', `Shard ${id} has resumed!`);
		});
